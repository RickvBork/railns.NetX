...
class track:
	'''
	Track class models a track, i.e. a single train riding between station on a 
	networkx graph.	Edges as well as critical edges that are traversed are 
	stored in list.
	'''
	def __init__(self, G):
	...
	def add_edge(self, edge):
		# check if current last station is starting point of new edge since 
		# trains cannot teleport
		if self.edges != []:
			# check if departing station is indeed the station where the train 
			# is 
			...
			if current_last_station == edge[0]:
				...
				print("Error, cannot depart from station where train is \
					located")
		else:
			self.edges.append(edge)	
			self.update_time()
	...
	def update_critical_lines_traversed(self):
		...
		for edge in edges:
			...
			if self.G[station_0][station_1]['color'] == 'r':
				# check if line is already in critical_lines_traversed, if not, 
				# add edge to critical_lines_traversed
				if (edge not in self.critical_lines_traversed and edge_reversed \
					not in self.critical_lines_traversed):
